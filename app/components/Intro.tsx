...
function Intro({ links, selectedPage }: { links: LINKS; selectedPage: string }): React.JSX.Element {
	...
	const [showCursorsClass, setShowCursorsClass] = useState<boolean[]>([false, false, false, false, false])
	...
	const introTextsData: React.JSX.Element[] = [
		...
		<>
			soy{' '}
			<motion.span {...animationProps} animate={{ color: 'var(--frontend-developer-color)' }}>
				frontend developer
			</motion.span>
		</>,

		<>
			si has llegado hasta mi web posiblemente sea porque quieres saber más{' '}
			<motion.span {...animationProps} animate={{ color: 'var(--about-me-color)' }}>
				<a
		...
		<>
			Aquí podrás encontrar un portafolio con todos los{' '}
			<motion.span {...animationProps} animate={{ color: 'var(--projects-color)' }}>
				<a
					onClick={scrollOnClick}
			...
			donde he trabajado, así como un detallado listado de mi&nbsp;{' '}
			<motion.span {...animationProps} animate={{ color: 'var(--experiences-color)' }}>
				<a
					onClick={scrollOnClick}
	...
	useEffect(() => {
		const showPrompt = async (dataKey: string): Promise<void> => {
			setIntroTexts(currentIntroTexts => {
				return { ...currentIntroTexts, [dataKey]: prompt }
			})
		...
		const showTextLetterByLetter = async (dataKey: string, stateIndex: number): Promise<void> => {
			...
			for (let i = 0; i < introTextDataString.length; i++) {
				setIntroTexts(currentIntroTexts => {
					return {
						...currentIntroTexts,
						[dataKey]: (currentIntroTexts[dataKey] as string) + introTextDataString[i]
					}
				})
	...
	// RENDER
	return (
		<div className='intro font-bold pl-2 pr-0 py-4 sm:p-6 text-lg sm:text-3xl flex-1 w-[22rem] sm:w-[36rem] min-h-[33rem] lg:min-h-auto'>
