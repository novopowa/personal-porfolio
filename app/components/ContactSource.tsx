	...
	const content = (): React.JSX.Element => {
		return props.link !== undefined ? (
			<a
				...
				target='_blank'
				className={`${major.className} text-sm sm:text-lg my-3 p-3 rounded-xl border-2 min-w-[20.5rem] sm:min-w-[25rem]`}>
				{props.icon}
				{props.name}
				<MdArrowOutward />
			</a>
			...
			<button
				className={`${major.className} text-sm sm:text-lg text-left my-3 p-3 rounded-xl border-2 min-w-[20.5rem] sm:min-w-[25rem]`}
